...
\begin{enumerate}
  \item[4.8.] {
      ...
      \begin{enumerate}
        ...
        \item if $f:X\to Y$ and $g:Y\to Z$ are two morphisms, and if $g\circ f$ has $\P$ and $g$ is seperated, then $f$ has $\P$;
...
\begin{proof}
  ...
  \end{align*}
  Next, take $g_1:Y\times_Z Y\xrightarrow{\pi_1} Y\xrightarrow{g} Z$, 
  \[
    \begin{tikzcd}
      X\ar[d,"f"']\ar[r,"\Gamma_f"']
      & X\times_Z Y\ar[d,"\mbox{$(f,\id)$}"]\ar[r] 
      & X\ar[d, "f\circ g"] \\
      Y\ar[r,"\Delta"'] & Y\times_Z Y\ar[r,"g_1"'] & Z
    \end{tikzcd}
  \] 
\end{proof}
