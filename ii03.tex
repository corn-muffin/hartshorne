...
\begin{proof}
  ...
  \begin{lemma}
    Fix a sheaf morphism $f:X\to Y$ which is locally of finite type, $\Spec A\cong U\subset X$, $\Spec B\cong V\subset Y$, $f(U)\subset Y$ as above, and denote the map of rings associated to $f|_U:U\to V$ by $\vphi:B\to A$.
...
\end{proof}
 ...
 \begin{proof}
   ...
   \end{align*}
   by quasi-compactness of $V$.
   Note that $D(g_{ij})=(V_i)_{g_{ij}}$, in the sense of (Ex. 2.16).
   Thus, we proceed via two brief lemmas.

   \begin{lemma}
     Let $f:X\to Y$ be a map of schemes.
     For any $g\in \Gamma(\O_Y,Y)$, we have
     \[
       f^{-1}(Y_g)\cong X_{f^\sharp(g)}.
     \] 
   \end{lemma}
   ...
   \begin{proof}
     Recall that
     \[
       Y_g = \{y\in Y~:~ f_y\notin \m_y\subset \O_{Y,y}\}.
     \] 
     Thus, we can write
     \[
       f^{-1}(Y_g) = \{x\in X ~:~f_{f(x)}\notin \m_{f(x)}\subset \O_{Y,f(x)}\}.
     \] 
     $f^\sharp_x:\O_{Y,f(x)}\to \O_{X,x}$ is a local morphism, so ${f^\sharp_x}^{-1}(\m_x)=\m_{f(x)}$.
     Hence, $h_{f(x)}\in \m_{f(x)}$ if and only if $f^\sharp_x(h_{f(x)})=f^\sharp(h)_x\in \m_x$.
     This gives us
     \[
       f^{-1}(Y_g)=X_{f^\sharp(g)},
     \] 
     as desired.
   \end{proof}

   \begin{lemma}
     Let $f:X\to Y$ be a map of schemes, and $\Spec B\cong V\subset Y$ an affine open with quasi-compact preimage $U:=f^{-1}(V)$.
     For any $g\in B$, $f^{-1}(D(g))$ is quasi-compact.
   \end{lemma}

   \begin{proof}
     Take a finite cover $\{U_i\}$ of $U$ by open affines.
     The proceeding lemma and (Ex. 2.16a) give us that
     \[
       f^{-1}(D(g))\cap U_i = U_{f^\sharp(g)}\cap U_i
     \] 
     is affine.
     Then 
     \begin{align*}
       f^{-1}(D(g)) &= \bigcup_{i=1}^n f^{-1}(D(g))\cap U_i
     \end{align*}
     is a finite union of quasi-compact spaces, and is thus quasi-compact.
   \end{proof}
   
   The above lemma gives us that each $f^{-1}(D(g_{ij}))\subset f^{-1}(V_i)$ is quasi-compact, whence $f^{-1}(V)$ is quasi-compact, as desired.
 \end{proof}
 ...
 \begin{proof}
   ...
   Suppose $f$ is locally of finite type and quasi-compact, and fix an open affine cover of $\{V_i\cong \Spec B_i\}$.
   By (Ex. 3.1) and (Ex. 3.2), each $f^{-1}(V_i)$ is quasi-compact and covered by open affines $U_{ij}\cong \Spec A_{ij}$ such that $A_{ij}$ is a finitely generated $B_i$-algebra.
   Hence, the $f^{-1}(V_i)$ are covered by finitely many such $U_{ij}$, and we're done.

   (b) This is a corollary of (a), (Ex. 3.1), and (Ex. 3.2).

   (c) This follows from Lemma 1 and Lemma 2, which were given in the proof of (Ex. 3.1).
 \end{proof}
 
 \begin{enumerate}
   \item[3.4.] {
       Show that a morphism $f:X\to Y$ is finite if and only if for \emph{every} open affine subset $V=\Spec B$ of $Y$, $f^{-1}(V)\cong \Spec A$ is affine with $A$ a finite $B$-module.
     }
 \end{enumerate}
...
\begin{enumerate}
	\item[3.9.] {
      ...
      \begin{enumerate}
        \item Let $k$ be a field, and let $\AA^1_k=\Spec k[x]$ be the affine line over $k$.
          Show that $\AA_k^1\times_{\Spec k}\AA_k^1\cong \AA_k^2$, and show that the underlying point set of the product is not the product of the underlying point set of the factors (even if $k$ is algebraically closed).
...
\begin{proof}
  (a) We have that $k[x]\otimes_k k[x]\cong k[x,y]$ as $k$-algebras, so $\AA_k^1\times_{\Spec k}\AA_k^1\cong \AA_k^2$ as schemes over $k$.
  Fix $k=\CC$, and consider the generic points of $\AA_k^2$.
  %If there were a bijection between $\AA_k^2$ and the set theoretic product, then this would give us a bijection of generic points, and thus a bijection of irreducible components.
  %There is an injection of point sets $f:\AA_k^1\times\AA_k^1\to \AA_k^2$ given by $(\p,\q)\mapsto \p^e + \q^e$.
  A bijection of sets would give us a bijection of generic points, and thus a bijection of irreducible polynomials.
  This is clearly false, so that $\AA_k^2\neq \AA_k^1\times \AA_k^1$ as sets.
...
\begin{proof}
  ...
  \begin{proof}
    ...
    \[
      ...
      \end{tikzcd}
    \] 
    By (Ex. 2.18), $\pi_A^*$ and $\pi_B^*$ are homeomorphisms onto their images $V(\a)$ and $V(\b)$, so that the result follows.
  \end{proof}
  ...
  \begin{align*}
    X_\p&=\Spec (B\otimes_A k(\p)) \\
       ...
       &\cong V(\p B_{A_\p}),
  \end{align*}
  where the last equality follows from (Ex. 2.18) as before.
  Then equalities
  \begin{align*}
       ...
       &\cong f^{-1}(\p).
  \end{align*}
  follow from minimality of the extension of an ideal, maximality of $\m_\p\lideal A_\p$, and the second lemma respectively.
  %\footnote{I originally wrote this out a while ago, so I hadn't written it up again, let alone in simplied form, when we had our meeting. For some reason I thought (b) was the interesting part lol.}
  ...
  \begin{align*}
    A_\eta &\cong \frac{k[s,t]}{(s-t^2)}\otimes_{k[s]}\frac{k[s]_{(0)}}{(0)k[s]_{(0)}} \\
           ...
           &\cong k(\sqrt{s}).
  \end{align*}
  $A_\eta$ is a field, so $\Spec A_\eta$ is singleton.
  Furthermore, it is a degree two extension over $k(\eta)=k(s)$, as desired.
\end{proof}
	...
	\item[3.11.] {
     ...
     \begin{enumerate}
       ...
       \item Let $f:Z\to X$ be a morphism.
         ...
         the morphism $f$ factors through $Y$, and if $Y'$ is any other closed subscheme of $X$ through with $f$ factors, then $Y\to X$ factors through $Y'$ also.
...
\begin{proof}
  (a) Take an affine open cover $\{X_i\cong \Spec A_i\}$ of $X$ and let $Y_i\cong \Spec B_i$, $X'_i\cong \Spec C_i$ be the inverse images of the $X_i$.
  This is wrong I think!
  $f|_{Y_i}:Y_i\to X_i$ corresponds to a map $\phi_i:$
  Then $Y\times_XX'$ is the gluing of the open affine sets 
  \[
    U_i:=Y_i\times_{X_i}X'_i\cong\Spec (B_i\otimes_{A_i}C_i),
  \]
  and $f'|_{U_i}:\Spec(B_i\otimes_{A_i}C_i)\to \Spec C_i$ corresponds to the map $\phi$
\end{proof}
...
\begin{proof}
  Let $\Sigma$ be the poset of closed subsets of $X$ such that $\P$ holds, which is nonempty since $\0\in \Sigma$.
...
\begin{enumerate}
  \item[3.20.] {
      \emph{Dimension}. Let $X$ be an integral scheme of finite type over a field $k$ (not necessarily algebraically closed).
