...
I couldn't keep doing problems that require 2.17 without proving it, or at least the meat of it in 2.16, for myself.
Also, I didn't quite have time to finish my write up of 5.17, and 2.16 seemed much more interesting than 5.3, for example.
I might try to go back and write up a couple more of these problems and some of the problems from 2 and 3 that I missed over the break.
These next couple of weeks will be very very rough, so I might have to just write up easy problems for next week.
We shall see\ldots
...
%         \item A schem $X$ is affine if and only if there is a finite set of elements $f_1,\ldots, f_r\in A=\Gamma(X,\O_X)$ such that the open subsets $X_{f_i}$ are affine, and $f_1,\ldots, f_r$ generate the unit ideal in $A$.
%       \end{enumerate}
%     }
% \end{enumerate}
% 
% \begin{proof}
%   (b) If $X$ is affine, then we have
% \end{proof}
